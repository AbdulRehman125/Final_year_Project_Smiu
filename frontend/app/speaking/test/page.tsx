...
// // Absolute floor for the "is this speech?" threshold. In a quiet room this is
// // what's used. In a noisy room we raise the threshold ABOVE the measured
// // ambient level (see SPEECH_MARGIN) so background noise is never mistaken for
// // the candidate still speaking — otherwise the mic never closes.
// const SILENCE_VOLUME_THRESHOLD = 12;
// // How far above the room's measured ambient floor a signal must rise to count
// // as real speech. Keeps steady background noise (fans, traffic, hum) below the
// // bar so silence is detected reliably.
// const SPEECH_MARGIN = 10;
// // Speech must persist for a couple of frames before it cancels a pending
// // silence close, so a single noise blip (a door, a cough) can't hold the mic
// // open indefinitely.
...
//   // Adaptive silence detection: the measured ambient noise floor for the
//   // current turn, and a counter of consecutive above-threshold frames.
//   const noiseFloorRef = useRef(0);
//   const speakFramesRef = useRef(0);

//   // ── Results-payload tracking ──────────────────────────────────────────
//   // The results screen needs the conversation transcript, how long the
//   // candidate actually spoke, how many responses they gave, and the test
//   // duration. We accumulate these via refs (to avoid stale closures in async
//   // WS/recorder callbacks) and embed them into `speaking_results`.
...
//   /**
//    * Show the RIGHT connection error. We don't trust `navigator.onLine` (it stays
//    * true on a LAN with no real internet), so we actively probe reachability and
//    * tell the candidate what actually happened — an internet problem vs. the
//    * examiner being unreachable — in plain, non-technical language.
//    */
//   const failConnection = useCallback(
//     async (kind: "connect" | "lost") => {
//       const online = await isInternetReachable();
//       if (!online) {
//         handleConnectionLost(
//           "No internet connection",
//           "It looks like your device is offline, so we couldn't reach the examiner. Please check your internet connection and try again — nothing is lost."
//         );
//       } else if (kind === "connect") {
//         handleConnectionLost(
//           "Can't reach the examiner",
//           "We couldn't connect to the speaking examiner just now. This is usually temporary — please try again in a moment."
//         );
//       } else {
//         handleConnectionLost(
//           "Connection lost",
//           "The connection to the examiner dropped, so your progress can't be resumed. Please start the test again."
//         );
//       }
//     },
//     [handleConnectionLost]
...
//         if (blob.size > 1000) {
//           // Count this as a real, evaluated response and add its duration to
//           // the running speaking-time total (used on the results screen).
//           responseCountRef.current += 1;
//           if (recordStartRef.current > 0) {
//             speakingMsRef.current += Date.now() - recordStartRef.current;
//           }
...
//       // Before the candidate starts speaking, sample the room's background
//       // level and take the median (robust to the odd spike). We then require
//       // real speech to rise clearly ABOVE that floor. This is the fix for the
//       // mic never closing in a noisy room: with a fixed threshold, steady
//       // background noise sat above it forever and looked like speech.
//       if (!hasSpokenRef.current) {
//         noiseSamplesRef.current.push(avg);
//         if (noiseSamplesRef.current.length > 20) noiseSamplesRef.current.shift();
//         const sorted = [...noiseSamplesRef.current].sort((a, b) => a - b);
//         const median = sorted.length ? sorted[Math.floor(sorted.length / 2)] : 0;
//         noiseFloorRef.current = median;
//         setNoiseLevel(median > 14 ? "HIGH" : "LOW");
//       }

//       // Speech must clear the room's floor by a margin — never below the
//       // absolute floor, so a silent room still works normally.
//       const threshold = Math.max(SILENCE_VOLUME_THRESHOLD, noiseFloorRef.current + SPEECH_MARGIN);
...
//         speakFramesRef.current = 0;
//       }
//       const speakingConfirmed = speakFramesRef.current >= SPEECH_CONFIRM_FRAMES;

//       if (speakingConfirmed) {
//         // Genuine speech (sustained above the ambient-aware threshold).
//         if (!hasSpokenRef.current) {
//           hasSpokenRef.current = true;
//           if (noSpeechTimer.current) { clearTimeout(noSpeechTimer.current); noSpeechTimer.current = null; }
//         }
//         // Speaking again → cancel any pending auto-close.
//         if (silenceTimer.current) { clearTimeout(silenceTimer.current); silenceTimer.current = null; }
//       } else if (hasSpokenRef.current && !silenceTimer.current) {
//         // Below the threshold after having spoken → start the silence
//         // countdown. Because the threshold sits above the room's ambient floor,
//         // background noise stays under it and no longer resets this timer, so
//         // the mic reliably closes ~POST_SPEECH_SILENCE_MS after the candidate
...
//         // Build the payload the results screen needs: the raw evaluation plus
//         // the conversation transcript and speaking metrics we've tracked. The
//         // raw result rarely carries these, so we attach them here.
...
//           transcript,
//           time_taken_seconds:
//             typeof rawResult.time_taken_seconds === "number"
//               ? rawResult.time_taken_seconds
//               : elapsedRef.current,
//           total_speaking_seconds:
//             typeof rawResult.total_speaking_seconds === "number"
//               ? rawResult.total_speaking_seconds
//               : Math.round(speakingMsRef.current / 1000),
//           response_count:
//             typeof rawResult.response_count === "number"
//               ? rawResult.response_count
...
//         let stored = true;
//         try {
//           sessionStorage.setItem("speaking_results", JSON.stringify(fullPayload));
//         } catch {
...
//           } else {
//             // Fallback: keep the URL short by omitting the (large) transcript.
//             const { transcript: _omit, ...slim } = fullPayload;
//             void _omit;
//             router.push(
//               `/speaking/results?data=${encodeURIComponent(JSON.stringify(slim))}`
//             );
//           }
//         } catch (e) {
...
//       setStatusBoth("idle");
//       void failConnection(wasConnected.current ? "lost" : "connect");
//     };
//   };
...
export default function SpeakingTestPage() {
  ...
  // ── Dark mode ──
  // Toggles the `dark` class on <html> (Tailwind's `dark:` variants respond to
  // it) and remembers the choice. SSR-safe: the initial read happens in an
  // effect so server and client render the same first paint.
  ...
  useEffect(() => {
    // The test is effectively over once we're evaluating or done — freeze the
    // clock so it doesn't keep ticking while we wait for the score.
    if (
      status === "idle" ||
      status === "connecting" ||
      status === "evaluating" ||
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      // 1) A truly closed socket is an instant, definitive drop.
      const sock = ws.current;
      const socketDead =
        !sock ||
        sock.readyState === WebSocket.CLOSING ||
      ...
      if (socketDead) {
        if (statusRef.current === "evaluating") {
          // On a SUCCESSFUL evaluation the message handler switches to "done"
          // and navigates to the results page BEFORE the socket closes. So a
          // dead socket that's still "evaluating" means scoring didn't finish
          // (the AI examiner most likely hit a rate limit or errored). We grace-
          // check once to rule out a close/message race, then surface a clear
          // message immediately instead of leaving the user stuck on
          // "Evaluating..." until the long (90s) watchdog fires.
          await new Promise((r) => setTimeout(r, 1500));
          if (cancelled || abortedRef.current) return;
          if (statusRef.current === "evaluating") {
            fail(
              "Couldn't finish scoring",
              "The examiner couldn't finish evaluating your test — it may be busy right now. Your answers weren't scored this time. Please try again in a moment."
            );
          }
          return;
        }
  ...
  const examinerStatusLabel =
    status === "examiner_speaking"
      ? { text: "SPEAKING", color: "text-blue-600 bg-blue-50 dark:bg-blue-500/15" }
      : status === "evaluating"
      ? { text: "THINKING", color: "text-amber-600 bg-amber-50 dark:bg-amber-500/15" }
      : { text: "LISTENING", color: "text-emerald-600 bg-emerald-50 dark:bg-emerald-500/15" };
  ...
  return (
    <div className="h-screen bg-gray-50 dark:bg-slate-950 flex flex-col overflow-hidden">
      <div className="w-full max-w-6xl mx-auto flex-1 flex flex-col px-4 py-4 gap-3 min-h-0">
        ...
        {/* ── Header — 3 cards ── */}
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl px-4 py-2.5 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-blue-500 text-white flex items-center justify-center shrink-0">
              <Mic className="w-4 h-4" />
            </div>
            <div>
              <p className="text-sm font-semibold text-gray-900 dark:text-slate-100 leading-tight">IELTS Speaking</p>
            ...
            <button
              onClick={toggleTheme}
              className="ml-2 w-8 h-8 rounded-full hover:bg-gray-100 dark:hover:bg-slate-800 flex items-center justify-center text-gray-400 dark:text-slate-500 dark:text-slate-500 shrink-0 transition-colors"
          ...
          <div className="flex-1 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl p-1.5 flex items-center gap-1.5">
            {(["part1", "part2", "part3"] as const).map((p) => (
              <div
                key={p}
                className={`flex-1 px-4 py-2 rounded-xl text-sm font-medium text-center transition-all flex items-center justify-center gap-2 ${
                  currentPart === p ? "bg-blue-600 text-white shadow-sm" : "text-gray-400 dark:text-slate-500"
                }`}
              >
                {p === "part1" ? "Part 1" : p === "part2" ? "Part 2" : "Part 3"}
                {currentPart === p && <span className="w-1.5 h-1.5 rounded-full bg-white/80" />}
          ...
          <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl px-5 py-2.5 flex items-center justify-between gap-4 min-w-[180px]">
            <div className="flex items-center gap-2">
              <Radio className="w-4 h-4 text-blue-500" />
              <div>
                <p className="text-lg font-bold text-gray-900 dark:text-slate-100 leading-tight tabular-nums">
                  {formatElapsed(elapsedSeconds)}
                </p>
                <p className="text-[10px] text-gray-400 dark:text-slate-500 uppercase tracking-wide leading-tight">
                  Elapsed
                </p>
              </div>
            </div>
        ...
        {(status === "idle" || status === "connecting") && (
          <div className="flex-1 min-h-0 flex items-center justify-center overflow-hidden py-3">
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 max-w-lg w-full text-center">
              <div className="w-12 h-12 rounded-2xl bg-blue-500 flex items-center justify-center mx-auto mb-3 shadow-lg shadow-blue-500/30">
                <User className="w-6 h-6 text-white" />
              </div>
              <h1 className="text-lg font-bold text-gray-900 dark:text-slate-100 mb-1.5">IELTS SPEAKING TEST</h1>
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                <div className="bg-gray-50 dark:bg-slate-950 rounded-xl px-2 py-2">
                  <p className="text-xs font-semibold text-blue-600 mb-0.5">Part 1</p>
                  <p className="text-xs text-gray-600 dark:text-slate-300">Interview</p>
                  <p className="text-[11px] text-gray-400 dark:text-slate-500 mt-1">4-5 min</p>
                </div>
                <div className="bg-gray-50 dark:bg-slate-950 rounded-xl px-2 py-2">
                  <p className="text-xs font-semibold text-blue-600 mb-0.5">Part 2</p>
                  <p className="text-xs text-gray-600 dark:text-slate-300">Long Turn</p>
                  <p className="text-[11px] text-gray-400 dark:text-slate-500 mt-1">3-4 min</p>
                </div>
                <div className="bg-gray-50 dark:bg-slate-950 rounded-xl px-2 py-2">
                  <p className="text-xs font-semibold text-blue-600 mb-0.5">Part 3</p>
                  <p className="text-xs text-gray-600 dark:text-slate-300">Discussion</p>
                  <p className="text-[11px] text-gray-400 dark:text-slate-500 mt-1">4-5 min</p>
                </div>
              </div>

              <div className="flex items-start gap-2.5 bg-blue-50/60 rounded-xl px-4 py-3 mb-4 text-left">
                <div className="w-5 h-5 rounded-full bg-blue-100 dark:bg-blue-500/20 flex items-center justify-center shrink-0 mt-0.5 text-blue-500 dark:text-blue-300 text-xs font-bold">
                  +
                </div>
              ...
              {status === "idle" ? (
                ...
                <div className="w-full bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400 font-medium py-3 rounded-xl text-sm flex items-center justify-center gap-2">
                  <Loader2 className="w-4 h-4 animate-spin" />
                  Connecting...
                </div>
              )}
        ...
        {status !== "idle" && status !== "connecting" && (
          <>
            <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3 min-h-0">
              ...
              {/* Examiner panel */}
              <div className="relative bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl flex flex-col min-h-[420px] max-h-[560px]">
                <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-slate-700">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">AI Examiner</p>
                    <span
                      className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full ${examinerStatusLabel.color}`}
                    >
                      {examinerStatusLabel.text}
                ...
                <div
                  className="flex-1 overflow-y-auto p-4 space-y-3 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300/70 [&::-webkit-scrollbar-thumb:hover]:bg-slate-400 dark:[&::-webkit-scrollbar-thumb]:bg-slate-600/70"
                  style={{ scrollbarWidth: "thin", scrollbarColor: "rgb(203 213 225 / 0.7) transparent" }}
                >
                  {messages.map((msg, i) => {
                    if (msg.role === "divider") {
                      return (
                        <div key={i} className="flex items-center gap-3 py-1">
                          ...
                          <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 dark:text-slate-500 whitespace-nowrap">
                            {msg.text}
                          </span>
                          <div className="flex-1 h-px bg-gray-200 dark:bg-slate-700" />
                        </div>
                        ...
                        <div className="max-w-[85%]">
                          {msg.role !== "system" && (
                            <p
                              className={`text-[10px] font-medium mb-1 ${
                                msg.role === "user"
                                  ? "text-right text-emerald-500 dark:text-emerald-400"
                          ...
                          <div
                            className={`inline-block rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                              msg.role === "examiner"
                                ? "bg-blue-50 dark:bg-blue-500/15 text-blue-900 dark:text-blue-100 rounded-tl-sm"
                                : msg.role === "user"
                                ? "bg-emerald-50 dark:bg-emerald-500/15 text-emerald-900 dark:text-emerald-100 rounded-tr-sm"
                                : "bg-amber-50 dark:bg-amber-500/15 text-amber-800 dark:text-amber-200 text-xs px-3 py-1.5"
                            }`}
                          >
                            {msg.text}
                            ...
                            {/* Structured Part 2 cue card */}
                            {msg.cueCard && (
                              <div className="mt-3 rounded-xl bg-white/70 dark:bg-slate-800/70 border border-blue-100 dark:border-blue-500/20 p-3">
                                <p className="text-[13px] font-semibold text-blue-900 dark:text-blue-100 mb-2">
                                  {msg.cueCard.title}
                                </p>
                                <p className="text-[11px] font-medium uppercase tracking-wide text-blue-400 dark:text-blue-300 mb-1.5">
                                  You should say
                                </p>
                                <ul className="space-y-1.5">
                                  {msg.cueCard.bullet_points.map((bp, bi) => (
              ...
              {/* Candidate panel */}
              <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl flex flex-col min-h-[420px] max-h-[560px]">
                <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-slate-700">
                  <div className="w-9 h-9 rounded-full bg-gray-100 dark:bg-slate-800 flex items-center justify-center shrink-0">
                    <User className="w-4.5 h-4.5 text-gray-500 dark:text-slate-400" />
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-900 dark:text-slate-100">Test Candidate</p>
                    <p className="text-xs text-gray-400 dark:text-slate-500">Your responses</p>
                  </div>
                  <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600">
                    <Wifi className="w-3 h-3" /> GOOD
                  </span>
                  <span
                    className={`flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full ${
                      noiseLevel === "LOW"
                        ? "bg-emerald-50 dark:bg-emerald-500/15 text-emerald-600"
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4 gap-4">
                  {status === "part2_prep" && part2Topic ? (
                    <div className="w-full">
                      ...
                      <p className="text-sm font-semibold text-gray-900 dark:text-slate-100 mb-3">
                        {part2Topic.title}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-slate-400 mb-2">You should say:</p>
                      <ul className="space-y-1.5 mb-4">
                        {part2Topic.bullet_points.map((bp, i) => (
                          <li key={i} className="text-sm text-gray-700 dark:text-slate-200 flex items-start gap-2">
                            <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-blue-400 shrink-0" />
                            <span>{bp}</span>
                          </li>
                        ))}
                      ...
                      <div className="text-center">
                        <p className="text-xs text-gray-400 dark:text-slate-500 mb-1">Preparation time</p>
                        <p
                          className={`text-3xl font-bold tabular-nums ${
                            prepTimeLeft <= 10 ? "text-red-500" : "text-blue-600"
                          }`}
            ...
            {/* Bottom bar */}
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-2xl px-4 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={skipQuestion}
                  disabled={status !== "user_speaking"}
                  className="flex items-center gap-1.5 text-sm font-medium text-gray-500 dark:text-slate-400 hover:text-gray-700 disabled:opacity-40 disabled:cursor-not-allowed px-3 py-2 rounded-lg hover:bg-gray-50 transition-colors"
                >
                  <SkipForward className="w-4 h-4" /> Skip
                </button>
                <button
      ...
      {banner && (
        ...
        >
          <div
            className="relative w-full max-w-sm bg-white dark:bg-slate-900 rounded-3xl shadow-2xl ring-1 ring-black/5 p-6 text-center animate-[popIn_150ms_ease-out]"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setBanner(null)}
              aria-label="Close"
              className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center text-gray-400 dark:text-slate-500 hover:text-gray-600 hover:bg-gray-100 transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            >
              {banner.kind === "warning" && !isOnline ? (
                <WifiOff className="w-6 h-6 text-amber-500 dark:text-amber-400" />
              ) : (
                <AlertTriangle
                  className={`w-6 h-6 ${
                    banner.kind === "error"
            ...
            <h2
              id="speaking-error-title"
              className="text-base font-bold text-gray-900 dark:text-slate-100 mb-1.5"
            >
              {banner.title}
            </h2>
            <p className="text-sm text-gray-500 dark:text-slate-400 leading-relaxed mb-5">{banner.message}</p>
            ...
            <div className="flex flex-col gap-2">
              ...
              <button
                onClick={() => setBanner(null)}
                className="w-full text-gray-500 dark:text-slate-400 hover:text-gray-700 hover:bg-gray-50 font-medium py-2.5 rounded-xl text-sm transition-colors"
              >
                Dismiss
              </button>
            </div>
