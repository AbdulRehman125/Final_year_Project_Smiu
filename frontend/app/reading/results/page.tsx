...
function ReadingResultsContent() {
  ...
  useEffect(() => {
    ...
    try {
      const stored = safeSessionStorage.getItem<ReadingEvaluationResponse>(
        "reading_results"
      );
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] dark:bg-background flex items-center justify-center p-6">
        <div className="text-center space-y-4">
          <p className="text-slate-500">Reading result data not found.</p>
  ...
  // Chart configuration & data for Recharts Time Distribution
  const totalMins = Math.max(results.timeTakenSeconds / 60, 1);
  const p1Mins = Number((totalMins * 0.32).toFixed(1));
  const p2Mins = Number((totalMins * 0.35).toFixed(1));
  const p3Mins = Number((totalMins * 0.33).toFixed(1));

  const chartData = [
    { passage: "Passage 1", minutes: p1Mins },
    { passage: "Passage 2", minutes: p2Mins },
    { passage: "Passage 3", minutes: p3Mins },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-background text-slate-900 dark:text-slate-100 py-8 px-4 md:px-6">
      <div className="mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Official Academic Reading Evaluation
              </p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-card border border-slate-200/80 dark:border-border/60 rounded-[24px] p-6 flex flex-col justify-between shadow-sm space-y-2">
            ...
            <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5 overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min(
                    (results.timeTakenSeconds / 3600) * 100,
                    100
                  )}%`,
                }}
              />
        ...
        {/* ── 4. Passage Performance & Recharts Time Distribution (2 Columns) ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {/* Passage Performance */}
          <div className="bg-white dark:bg-card border border-slate-200/80 dark:border-border/60 rounded-[24px] p-6 shadow-sm space-y-3">
            <h3 className="text-xs font-black uppercase tracking-wider text-slate-800 dark:text-slate-100">
